  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className="w-full flex items-center gap-1.5 bg-surface border border-border/60 rounded-xs px-2 py-1.5 text-foreground focus:outline-none focus:ring-1 focus:ring-primary dark:bg-sidebar!"
      >
        {value ? (
        ...
        <span className="flex-1 text-left truncate text-[11px]">{value ? (selected?.name ?? "현재 텍스처") : "없음 (색상)"}</span>
