...
export function regenerateCloner(
  ...
  const count = Math.max(2, Math.min(200, Math.round(cfg.count)));
  ...
  for (let i = 1; i < count; i++) {
    ...
    additions.push({ ...source, id: rootNewId, parentId: clonerGroupId, clonerClone: true, position: { x: pi.x, y: pi.y, z: pi.z }, name: `${source.name} ${i}` });
