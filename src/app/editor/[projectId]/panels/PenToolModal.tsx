...
export function PenToolModal() {
  ...
  const undoStep = () => {
    if (closedRef.current) setClosed(false);
  ...
  const onPointDown = (e: React.MouseEvent, idx: number) => {
    ...
    if (idx === 0 && !closed && points.length >= 2) {
      setClosed(true);
      return;
    }
  ...
  const onSvgMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    let dx = p.x - d.startMouse.x,
      dy = p.y - d.startMouse.y;
    if (snapGrid) {
  ...
  const onSvgUp = () => {
    ...
    if (boxRef.current && box) {
      ...
      points.forEach((pt, i) => {
        if (pt.x >= box.x0 && pt.x <= box.x1 && pt.y >= box.y0 && pt.y <= box.y1) inBox.add(i);
      });
      setSelected(inBox);
    }
    boxRef.current = null;
  ...
  const create = () => {
    if (mode === "extrude" && points.length < 3) {
      addToast("돌출은 점 3개 이상 필요해요.", "error");
      return;
    }
    if (mode === "lathe" && points.length < 2) {
      addToast("회전체는 점 2개 이상 필요해요.", "error");
      return;
    }
    if (mode === "extrude" && !closed) {
      addToast("첫 점을 클릭해 경로를 닫아주세요.", "error");
      return;
    }
  ...
  const switchMode = (m: "extrude" | "lathe") => {
    if (m === mode) return;
    setMode(m);
    clearAll();
  };
  ...
  return (
    <>
      <div
        ref={panelRef}
        className="fixed z-50 bg-surface border border-border rounded-sm shadow-2xl p-4 w-[500px] max-w-[97vw] max-h-[92vh]"
      ...
      >
        <div className="flex items-center justify-between mb-2.5 cursor-move select-none" onMouseDown={onHeaderDown}>
          <span className="text-[13px] font-semibold text-foreground flex items-center gap-1.5">
            <PenTool size={14} /> {editId ? "펜 툴 — 오브젝트 수정" : "펜 툴 — 2D 그려서 3D 만들기"}
          </span>
          <button onMouseDown={(e) => e.stopPropagation()} onClick={close} className="text-muted hover:text-foreground px-1 cursor-pointer">
            <X size={15} />
          </button>
        </div>
        ...
        <div className="flex gap-1.5 mb-2">
          <div className="grid grid-cols-2 gap-1 flex-1">
            {(["extrude", "lathe"] as const).map((m) => (
              ...
              >
                {m === "extrude" ? "돌출 (Extrude)" : "회전체 (Lathe)"}
              </button>
            ))}
          </div>
          <button
          ...
          >
            <span className="inline-flex items-center gap-1">곡선 {smooth && <Check size={11} />}</span>
          </button>
          <button
          ...
          >
            <span className="inline-flex items-center gap-1">스냅 {snapGrid && <Check size={11} />}</span>
          </button>
        </div>
        ...
        <svg
          ...
          className="mx-auto block rounded-xs bg-background border border-border cursor-crosshair select-none"
          // 정사각 유지 + 뷰포트 높이에 맞춰 축소(모달 너비만 따라 커져 하단이 잘리던 문제 방지).
          //   너비 = min(모달 폭, 사용 가능한 세로 높이). 헤더·버튼·안내문(≈230px)을 뺀 값.
          style={{ aspectRatio: "1 / 1", width: "min(100%, calc(90vh - 230px))" }}
        >
          {gridLines}
          ...
          {mode === "lathe" && previewPts.length > 1 && (
            <polyline
              points={previewPts.map((p) => `${(2 * CENTER - p.x).toFixed(1)},${p.y.toFixed(1)}`).join(" ")}
              fill="none"
              stroke="#a78bfa"
              strokeWidth={1.5}
              strokeDasharray="3 3"
        ...
        <p className="text-[10px] text-muted/70 dart:text-muted mt-1.5 leading-relaxed">
          {mode === "extrude"
          ...
          점 드래그로 조정. <b>Ctrl+클릭</b>=변에 점 추가, <b>Shift+드래그</b>=여러 점 선택→함께 이동, <b>Delete</b>=선택 삭제, <b>Ctrl+Z</b>=점 취소
          {closed ? " · 닫힘" : ""}.
        </p>
        ...
        <div className="flex gap-1.5 mt-3">
          <button
            ...
            className="flex-1 py-1.5 rounded-xs bg-background text-muted hover:text-foreground text-[11px] transition-colors disabled:opacity-40"
          >
            {closed ? "닫기 취소" : "점 취소"} (Ctrl+Z)
          </button>
          <button
            onClick={clearAll}
            disabled={points.length === 0}
          ...
          >
            {editId ? "수정 적용" : "만들기"} ({points.length})
          </button>
        </div>
      </div>
