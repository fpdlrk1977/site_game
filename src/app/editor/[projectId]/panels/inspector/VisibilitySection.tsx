...
export function VisibilitySection({ obj, open, onToggle }: { obj: ObjectNodeSchema; open: boolean; onToggle: () => void }) {
  ...
  return (
    <GroupBox>
      <SectionHeader
        title="Visibility"
        hint="Show/hide and lock, plus (primitives) shading, double-sided and shadow options. Hidden objects are hidden in the viewer too; locking blocks selection and moving in the viewport (still selectable in the hierarchy list)."
        isOpen={open}
        onToggle={onToggle}
      />
      {open && (
        <div className="px-3 pb-4 space-y-2">
          {(["visible", "locked"] as const).map((key) => (
            <label key={key} className="flex items-center justify-between cursor-pointer">
              <span className="text-[10px] text-muted/70 dark:text-muted capitalize">{key === "visible" ? "Visible" : "Locked"}</span>
              <Toggle
                value={obj[key]}
                onChange={() => {
                  updateObject(obj.id, { [key]: !obj[key] });
                  pushHistory();
                }}
              />
            </label>
