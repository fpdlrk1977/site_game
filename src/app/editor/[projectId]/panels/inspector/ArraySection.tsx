...
export function ArraySection({ obj, open, onToggle }: { obj: ObjectNodeSchema; open: boolean; onToggle: () => void }) {
  ...
  const onCols = (v: number) => {
    const n = clampN(v);
    setGridCols(n);
    if (linkCR) setGridRows(n);
  };
  const onRows = (v: number) => {
    const n = clampN(v);
    setGridRows(n);
    if (linkCR) setGridCols(n);
  };
        ...
        <div className="px-3 pb-4 space-y-2">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div className="flex-1">
              ...
              <RangeSlider
                value={arrayCount}
                onChange={(v) => setArrayCount(Math.max(1, Math.min(100, Math.round(v))))}
                min={1}
                max={100}
                step={1}
                showValue
                precision={0}
                disabled={arrayMode === "grid"}
              />
            </div>
          </div>
              ...
              <div className="flex gap-3 items-center">
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  ...
                  <div className="flex items-end gap-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <LabeledNum label="Rows" value={gridRows} onChange={onRows} onCommit={() => {}} min={1} max={50} precision={0} dragStep={1} />
                    </div>
                    <LinkToggle
                      value={linkCR}
                      onChange={(v) => {
                        setLinkCR(v);
                        if (v) setGridRows(gridCols);
                      }}
                      title="열=행 함께 조절"
                    />
                  </div>
                  ...
                  <div className="flex items-end gap-1">
                    <div className="flex-1 min-w-0">
                      <LabeledNum label="Col spacing" value={gridSpacing.x} onChange={onSpX} onCommit={() => {}} precision={2} dragStep={0.25} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <LabeledNum label="Row spacing" value={gridSpacing.z} onChange={onSpZ} onCommit={() => {}} precision={2} dragStep={0.25} />
                    </div>
                    <LinkToggle
                      value={linkSp}
                      onChange={(v) => {
                        setLinkSp(v);
                        if (v) setGridSpacing((s) => ({ ...s, z: s.x }));
                      }}
                      title="가로=세로 간격 함께 조절"
                    />
                  </div>
                </div>
              </div>
                ...
                <XYZRow
                  ...
                  rowEnd={
                    <LinkToggle
                      ...
                      onChange={(v) => {
                        setLinkSpL(v);
                        if (v) setArrayOffset((o) => ({ x: o.x, y: o.x, z: o.x }));
                      }}
                      title="세 축 함께 조절"
                    />
                  }
                ...
                <>
                  <div>
                    <LabeledNum
                      label="Radius"
                      value={arrayRadius}
                      onChange={(v) => setArrayRadius(Math.max(0.1, v))}
                      onCommit={() => {}}
                      min={0.1}
                      max={100}
                      precision={2}
          ...
          <Tooltip wide className="w-full" content="Creates independent objects now. The count can't be changed later (they become normal objects).">
            ...
            >
              <span className="inline-flex items-center gap-1.5">
                {arrayMode === "radial" ? <CircleDot size={13} /> : arrayMode === "grid" ? <Grid2x2 size={13} /> : <Rows3 size={13} />} Duplicate once
                ({total}, independent)
              </span>
            </button>
          </Tooltip>
          {!obj.parentId && (
            <Tooltip wide className="w-full" content="Change count and spacing anytime, and editing the source updates all copies in real time.">
              ...
              >
                <span className="inline-flex items-center gap-1.5">
                  <Grid2x2 size={13} /> Make live cloner ({total})
                </span>
              </button>
            </Tooltip>
          )}
