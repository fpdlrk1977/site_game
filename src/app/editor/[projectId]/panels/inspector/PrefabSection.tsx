...
import { X, CirclePile, Focus, Unlink } from 'lucide-react';
...
export function PrefabSection({ obj, open, onToggle }: { obj: ObjectNodeSchema; open: boolean; onToggle: () => void }) {
  const { objects, prefabs, createPrefab, applyInstanceToPrefab, revertInstance, setPrefabMaster, detachPrefabInstance } = useSceneStore();
  const { addToast } = useToast();
          ...
          const hasMaster = !!prefabDef?.masterInstanceId;
          const isMaster = hasMaster && prefabDef!.masterInstanceId === obj.prefabInstanceId; // 이 인스턴스가 원본
                ...
                {isInstance && instanceRoot && (
                  <>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[11px] font-semibold flex-1 truncate flex items-center gap-1.5" style={{ color: 'var(--prefab)' }}>
                        {isMaster ? <CirclePile size={12} className="shrink-0" /> : <Focus size={12} className="shrink-0" />} {prefabDef!.name}
                      </span>
                    ...
                    <div className="flex items-center gap-1.5">
                      {isMaster ? (
                        <span className="px-1.5 py-0.5 rounded-xs text-[10px] font-semibold text-white" style={{ background: 'var(--prefab)' }}>Master · 원본</span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded-xs text-[10px] font-semibold border" style={{ color: 'var(--prefab)', borderColor: 'var(--prefab)' }}>Copy · 사본</span>
                      )}
                    </div>

                    {isMaster ? (
                      /* 원본 — 편집하면 사본에 자동 반영. Apply/Revert 없음. */
                      <p className="text-[10px] text-muted/70">This is the master. Editing it updates every copy automatically — no Apply needed.</p>
                    ) : (
                      <>
                        {overrides.length > 0 ? (
                        ...
                        <div className={`grid ${hasMaster ? 'grid-cols-2' : 'grid-cols-3'} gap-1 pt-0.5`}>
                          {/* 원본 미지정(레거시) 프리팹만 수동 Apply 노출. 원본이 있으면 원본 편집이 곧 전파. */}
                          {!hasMaster && (
                            <button
                          ...
                          <button
                            onClick={() => { setPrefabMaster(instanceRoot.id); addToast('Set as master (copies now follow this)', 'success'); }}
                            title="Make this instance the master — from now on editing it auto-updates all copies"
                            className="py-1.5 rounded-xs text-white text-[11px] font-semibold transition-colors"
                            style={{ background: 'var(--prefab)' }}
                          >
                            Set as master
                          </button>
                          <button
                        ...
                        <button
                          onClick={() => { detachPrefabInstance(instanceRoot.id); addToast('Detached — now a plain group', 'success'); }}
                          title="Unlink this copy from the prefab → becomes a plain group (default color). The master and other copies are unaffected."
