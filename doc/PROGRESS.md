...
- **팝업 고도화 — 웹사이트/HTML 임베드 + 커스터마이징 (미착수, 설계만 — 2026-07-08 논의)**: 현재 `show_popup`은 `RichContent`로 이미지/영상/YouTube/Vimeo 자동 인식은 되나 **일반 웹사이트 URL은 링크로만** 뜨고 커스텀 HTML/스타일 불가 → "웹사이트도 품는다"는 취지에 부족. **방향(합의): iframe으로 통일** — URL 모드=`<iframe src>`, HTML 모드=`<iframe srcdoc sandbox>`(dangerouslySetInnerHTML의 XSS·스타일오염을 샌드박스로 회피), auto=기존 RichContent. **스키마 제안**(하위호환: popup 없으면 기존과 동일): `EventSchema.popup?: { mode:'auto'|'url'|'html'; width?; height?(px|%/vw/vh); bg?; padding?; title?; chrome?:boolean; position?:'center'|'left'|'right'|'bottom' }`, `value`=내용 유지. **⚠️ 함정**: 많은 사이트가 X-Frame-Options/CSP로 iframe 차단(빈 화면) → **로드 실패 감지 후 "삽입 불가 + 새 탭 열기" 폴백** 필수(대안=기존 open_url). 모바일은 vw/vh·%·max 클램프. **Phase 1**(핵심): mode(auto/url/html)+iframe+기본 스타일(width/height/bg/title)+차단 폴백+로딩 스피너, 에디터 Events에 '팝업' 접이식 섹션(모드 드롭다운→내용 입력→스타일). **Phase 2**: 위치 프리셋(모달/사이드패널/바텀시트), **씬 전역 기본 팝업 스타일 + 오브젝트별 오버라이드**(interactRange 패턴 재사용), 등장 애니메이션(fade/scale). UI 안내문: HTML="샌드박스 격리", URL="일부 사이트 삽입 차단". **착수 여부 미정(사용자 고민 중)**.
