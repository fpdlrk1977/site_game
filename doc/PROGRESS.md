...
- **⚠️ 성능 — 처음엔 심하게 버벅였다(사용자 보고). 원인 3종이 겹쳤다:**
  1. **SSAO(N8AO)** 4개 템플릿 전부 켬 — 가장 비싼 후처리.
  2. **`contactShadows: true`** — `ViewerCanvas`의 ContactShadows에 **`frames` 제한이 없어 매 프레임 1024 해상도로 전체를 다시 렌더**한다. 실제 그림자가 있으면 중복이기도 하다.
  3. **castShadow 스포트를 템플릿당 4개** — 광원마다 그림자맵을 매 프레임 렌더.
  → **SSAO 전부 제거 · contactShadows 전부 off · 그림자는 키 라이트 1개만**. `spot()` 기본 `castShadow:false`로 바꾸고 주석에 경고. 빛의 인상은 유지(개수·색·강도 그대로, 그림자맵만 감축).
  - **남은 앱 전역 개선 후보**: `ContactShadows`에 `frames` 제한 추가(정적 씬에선 1회면 충분). 단 모션·관절 오브젝트가 있으면 그림자가 멈춘 채 남으므로 별도 검토 필요.
