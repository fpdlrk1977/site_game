...
- **STEP 4 — 시그니처 모먼트 (진행 중)**: ✅ **빈 상태 코칭** 완료 — `panels/EditorEmptyState.tsx`(신규). `objects.length===0`이면 뷰포트 중앙에 카드(상자/구체/원기둥 즉시추가 `addObject` + 툴바·Ctrl+K 안내), 오브젝트 생기면 자체 숨김(objects 길이만 select). `EditorClient`가 캔버스 위 z-10에 렌더. 진단 클린+`✓ Compiled`. **브라우저 확인 대기.** 남은 STEP 4 항목(각각 대형 독립 기능): 인에디터 ▶ 플레이 토글 · 문장형 이벤트 UI · 오브젝트 프리셋(바퀴/문/동전).
