...
- **결과(현재)**: InspectorPanel.tsx **3,855 → 1,469줄(−62%)**. 신규 파일 16개(ui·Environment·GlbClipPicker + 섹션 13개). InspectorPanel 미사용 import·이동 상태/헬퍼 정리. tsc 클린 + editor 컴파일 200(SSR 정상).
- **남음(4단계 마무리 — 하드 2종)**: **다중선택 패널**(일괄편집/Merge/Boolean/정렬/Prefab라이브러리 — async handleMerge/handleBoolean·busy state 동반) · **EventsSection**(고난도 — 이벤트 폼 상태 10여개·핸들러(addEvent/startEdit/cancelEventForm/removeEvent)·600줄 renderEventForm·previewPopup 오버레이·이벤트 리스트·대화(dialogue) UI가 InspectorInner 곳곳 분산, 전용 신중 패스). 이후 InspectorPanel 잔여 미사용 import(THREE·worldBBox·localBBox·glbLocalBboxCache 등) 최종 정리. **순수 리팩터라 브라우저 확인하며 진행 중** — Material(텍스처 업로드/타일), Transform(위치·크기·바닥에놓기) 실동작 확인 권장.
