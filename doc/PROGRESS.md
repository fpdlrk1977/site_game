...
- **4단계 진행 — 섹션 컴포넌트화** (사용자 브라우저 확인: Motion/Environment 정상). **패턴 확립**: 부모가 guard(`{cond && <XSection obj={obj} open={isOpen('x')} onToggle={()=>toggleSection('x')}/>}`)만 유지, 섹션은 `<GroupBox>` 반환 + 스토어 직접 select + 접힘은 open/onToggle prop.
  - 추출 완료 섹션: **Motion·Physics·Light·Content·Particle·Visibility·Subdivision·Geometry·Cloner·Array·Prefab·Transform**(각 자체 파일). GlbClipPicker(공용)도 별도. Array는 로컬 상태(개수/간격/모드/반경/축), Transform은 setPos/setRot/setScl·바닥스냅 헬퍼, Prefab은 OVERRIDE_LABELS도 함께 이동.
...
- **결과(현재)**: InspectorPanel.tsx **3,855 → 1,661줄(−57%)**. 신규 파일 15개(ui·Environment·GlbClipPicker·Motion·Physics·Light·Content·Particle·Visibility·Subdivision·Geometry·Cloner·Array·Prefab·Transform). InspectorPanel 미사용 import·이동 상태/헬퍼/OVERRIDE_LABELS 정리. tsc 클린 + editor 컴파일 200(SSR 정상).
- **남음(4단계 마무리)**: **Material**(texPanelOpen/handleObjectTexUpload 텍스처 업로드 헬퍼 함께 이동) · **다중선택 패널**(일괄편집/Merge/Boolean/정렬/Prefab라이브러리 — async handleMerge/handleBoolean 동반) · **EventsSection**(고난도 — 상태·핸들러·600줄 form·preview·list가 InspectorInner 곳곳 분산, 전용 신중 패스). 이후 InspectorPanel의 잔여 미사용 import(THREE·worldBBox 등) 최종 정리. **순수 리팩터라 브라우저 확인하며 진행 중** — 특히 Transform은 전 오브젝트 공용이라 위치/크기/바닥에놓기 실동작 확인 권장.
