...
- **✅ 디자인 세련화 패스 (2026-07-25)**: 사용자 "라운드 xs 일괄 + 퀄리티 세련되게". 에디터 제외 전 페이지.
  - **라운드 `xs`(6px) 일괄** — 비-에디터 파일 전부 `rounded-(2xl/xl/lg/md/sm)`+방향변형 → `rounded-xs`(sed). 태그/필터 칩도 `rounded-full`→`rounded-xs`(기하학적·Linear 톤). 아바타·상태 dot만 원형 유지. **토큰(`--radius-xs`) 무변경**(에디터 무영향).
  - **큰 블러 blob 제거 → `components/ui/AmbientBackground.tsx`**: 미세 그리드(상단 페이드) + 소프트 글로우 1개. 랜딩·요금제·커뮤니티·인증(AuthShell)·계정 전부 교체("AI 티" blob 걷어냄).
  - **이모지 → lucide 라인 아이콘**: 랜딩 기능(🎯🎮🚀→MousePointerClick/Gamepad2/Rocket)·▶→Play·커뮤니티 ♡⇄→Heart/Repeat2·빈상태 🌌→Telescope/Box. (브랜드 ⬡만 유지.)
  - **딥 팔레트 공용화 `lib/thumbGradient.ts`**: 파스텔 대신 깊고 절제된 그라데이션 6종 → ProjectCard·커뮤니티 갤러리·작품상세·랜딩 갤러리 공유(중복 제거).
  - **그라데이션 절제**: 브랜드 마크 + 주 CTA에만 그라데이션, 보조 버튼은 테두리+subtle. 최종 CTA도 loud 그라데이션 블록 → surface 위 은은한 틴트. 배지=유리 pill+dot. 기능 아이콘=primary 틴트 사각. tsc 클린 + 전 라우트 200.
