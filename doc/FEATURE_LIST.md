...
### FEAT-GROUP-01: 그룹 선택 + 오브젝트 정렬
**설명**: 여러 오브젝트를 동시에 선택하여 일괄 조작하고, 축 기준으로 정렬한다.

**AC:**
- [ ] Shift+클릭으로 여러 오브젝트를 동시 선택할 수 있다
- [ ] 뷰포트 빈 공간을 드래그하면 박스 영역 안의 오브젝트가 일괄 선택된다
- [ ] 다중 선택 상태에서 기즈모 이동 시 선택된 모든 오브젝트가 함께 이동한다
- [ ] 다중 선택 상태에서 `Delete` 입력 시 선택된 모든 오브젝트가 삭제된다
- [ ] Inspector 상단에 "정렬" 버튼이 표시되며 X/Y/Z 축 기준 좌/중/우 정렬이 동작한다
...
### FEAT-LAYER-01: 레이어 시스템
**설명**: 오브젝트를 레이어로 분류하여 그룹 단위로 숨김/잠금/선택한다.

**AC:**
- [ ] Hierarchy 상단에 레이어 목록 탭이 표시된다
- [ ] Inspector에서 선택된 오브젝트의 레이어를 변경할 수 있다
- [ ] 레이어 단위 숨김 토글 시 해당 레이어 전체 오브젝트의 `visible`이 일괄 변경된다
- [ ] 레이어 단위 잠금 토글 시 해당 레이어 전체 오브젝트의 `locked`가 일괄 변경된다
- [ ] 새 레이어 추가 및 이름 변경이 가능하다
...
### FEAT-PARTICLE-01: 파티클 이미터
**설명**: Three.js Points 기반 파티클 효과를 씬에 배치한다.

**AC:**
- [ ] AssetBrowser "콘텐츠" 탭에 "파티클" 항목이 있으며 씬에 배치할 수 있다
- [ ] Inspector에서 파티클 수, 크기, 색상, 속도, 방향, 수명을 편집할 수 있다
- [ ] 불꽃 / 먼지 / 빛 입자 / 눈 프리셋이 제공된다
- [ ] 파티클 설정은 ObjectNodeSchema의 `particleConfig` 필드에 저장된다
- [ ] 뷰어에서 동일한 파티클 효과가 재생된다
