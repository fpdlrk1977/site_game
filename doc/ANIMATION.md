...
- [x] **P4 타임라인 UI (2026-07-14)**: 인스펙터 Animation에 컴팩트 **타임라인 바** — 키프레임을 시간축에 다이아몬드로. **클릭=그 포즈로 이동**(goToPose·하이라이트), **키 드래그=시간 이동**(setPoseTime 재사용, 이웃 사이 클램프, 마지막 키는 여유+0.5반올림 스케일로 지터↓), **빈 바 드래그=스크럽**(재생헤드를 끌어 중간 동작 미리보기).
  - **스크럽**: 스토어 transient `animScrub{clipId,t}` + `setAnimScrub`. `ClipPreview`(EditorCanvas)를 재생/스크럽 공용으로 일반화 — `animScrub`면 고정 t로 `sampleClip` 적용(비파괴 ref 구동, 종료 시 복원). 스크럽은 objects/스토어 안 건드려 **오토키 무발동**·재생과 상호배타. 재생헤드는 스크럽 중 t, 아니면 현재 편집 포즈 시간 표시.
  - **모드 토글(2026-07-14, 사용자 "원래 계획은 간단/타임라인 두 모드")**: 스토어 `animMode:'simple'|'timeline'`(UI 선호, 기본 simple). **간단**=인스펙터 세로 포즈 리스트(초보). **타임라인**=아래 참고 하단 패널. 인스펙터 헤더에 토글, 공통 컨트롤(트랙·이징·loop·경첩)은 양 모드 공유.
  - **하단 타임라인 패널로 재구성(2026-07-14, 사용자 "계획 아티팩트와 다르다")**: 처음엔 좁은 인스펙터 안 스트립으로 만들었으나 원래 시안은 **전체폭 바닥 패널**(트랜스포트+트랙 라벨+눈금자+레인+재생헤드)이었음 → 재작성. 신규 `panels/TimelinePanel.tsx`(EditorClient 하단 마운트, 타임라인 모드+선택 클립일 때). 열리면 `bottomInset`으로 레일/좌패널/인스펙터 바닥을 올려 전체폭 확보. 구성: **트랜스포트 바**(▶/⏹·loop·시간 readout·+키프레임·이징) + **좌측 라벨 컬럼**(오브젝트별 트랙 행, 클릭=selectObject 동기·선택 행 앰버) + **레인**(눈금자 0.5s + 트랙별 ◆ 키프레임 + 재생헤드). ◆ 클릭=goToPose·드래그=리타임, 빈 레인 드래그=스크럽(`animScrub`). 인스펙터는 타임라인 모드일 때 "하단에서 편집" 안내만. **적응**: 우리 포즈는 트랙 정렬형(공유 시간)이라 행마다 같은 x — 계획의 per-속성/per-트랙 독립 타이밍은 후속.
  - **타임라인 UX 다듬기(2026-07-15, 사용자 피드백)**: ①**화면 분리** — 타임라인 모드에선 인스펙터의 포즈 리스트·포즈추가·트랙목록/추가 전부 숨김(간단 모드 전용), 인스펙터는 클립 설정(경첩·이징·loop)만. 포즈/트랙 추가는 **전부 타임라인에서**. ②**+오브젝트(트랙)를 타임라인 라벨 컬럼**으로. ③**눈금자/레인 우클릭 → "여기에 키프레임 추가"**(그 시간에 삽입+선택 → 오브젝트 옮기면 오토키로 반영). `+키프레임`은 끝에 추가. ④**눈금자 고정 스케일** — 키 드래그 중 `dragScale`로 스케일 동결(예전엔 마지막 키 끌면 눈금자가 막 흔들림). ⑤**재생헤드 라인은 스크럽 중에만** 표시(평소엔 선택 ◆ 앰버가 현재 포즈 — 라인이 ◆과 겹쳐 헷갈리던 것 제거).
  - **③ 재생 중 재생헤드 스윕(2026-07-15)**: TimelinePanel이 `previewing`일 때 rAF로 경과시간→`playT`(loop/clamp) 갱신 → 재생헤드가 좌→우로 흐름(표시용, ClipPreview 구동과 별개). `headT`=스크럽||재생.
  - **① 이징 곡선 편집(2026-07-15)**: `EasingType`(linear/easeIn/easeOut/easeInOut/backOut/bounceOut) — `animSample.ts`에 곡선 함수맵. **구간(키프레임)별 이징** `AnimKeyframe.easing?`(그 키→다음 키 구간, 미설정 시 클립 기본 폴백). 클립 기본 `AnimClip.easing`도 6종으로 확장(스키마 widen, 하위호환). UI: 타임라인 트랜스포트에 **기본 곡선 + (키 선택 시)구간 곡선** 드롭다운, 인스펙터 이징도 6종. 결정적 테스트 17/17(끝점·키override). 
  - **타임라인 줌·호버·정리(2026-07-15, 사용자 피드백)**: ①**호버 시간 툴팁** — 레인 위 마우스 이동 시 커서에 얇은 가이드선+시간 라벨(`hoverT`, 드래그·재생 중엔 숨김). ②**휠 줌 — 초당 픽셀(pxPerSec) 모델**(사용자 "옵션 말고 그냥 늘어나게"): 위치=`t×pxPerSec` 절대 px. **표시 시간창 `viewMaxT=max(클립끝+0.5s, 컨테이너폭/pxPerSec)`** → 줌아웃하면 빈 미래 시간이 넉넉히 늘어나 먼 시간에도 키 배치 가능(하드 캡 없음, MIN_PPS=12→매우 김). 콘텐츠폭=`viewMaxT×pxPerSec`(가로 스크롤), 커서 밑 시간 고정(native wheel+useLayoutEffect scrollLeft). **절대 px라 키 드래그 시 눈금 안 흔들림**(dragScale 동결 제거). `ResizeObserver`로 뷰 폭 추적. 눈금 간격은 pxPerSec 기반 nice-step(~70px). `tFromX`/재생헤드/◆ 통일.
    - **무한 확장(2026-07-15, 사용자 "무한대라고 했잖아, 95s에서 막힘")**: 원인=MIN_PPS(줌 하한)가 최대 표시시간(뷰폭/MIN_PPS)을 캡. → ①**MIN_PPS 12→4**(줌아웃 개요 ~4.75분 뷰) + ②**스크롤 확장 `extraT`** — 항상 오른쪽에 ~1.5화면 여유를 두고, 오른쪽으로 스크롤하면 `extraT`가 계속 커져 **하드 캡 없이 3분·10분 자유**(`viewMaxT=max(클립끝+0.5, 뷰/pps, extraT)`, onScroll 확장, 단조증가 가드).
    - **눈금 가상화·단위 적응(2026-07-15, 사용자 "850s 눈금이 다 그려져 버벅, 단위가 안 변함")**: 원인=눈금을 0~viewMaxT 전체 생성(850s×0.25s=수천 개). → **보이는 구간(scrollLeft~+뷰폭)만 생성**(가상화, 화면당 ~15~21개 고정, 시간 길어도 렌더 가벼움·결정적 검증 ≤21). 단위는 pxPerSec 기반 nice-step(0.05s~30분) 자동(확대=0.1s, 축소=30s·1분·5분·10분), MIN_PPS 4→**0.5**(최소줌 ~38분 뷰). 큰 시간 라벨은 **분:초(2:57.25)**. **⏮ 처음으로(0s)** 버튼 추가(먼 곳에서 초기 복귀). extraT 부트스트랩 제거(줌아웃 시 콘텐츠 비대·5M px div 방지 — 줌아웃만으로 먼 시간 보이고 extraT는 스크롤로만 증가).
    - **뷰포트(창) 모델 전면 재작성(2026-07-15, 사용자 "레인 폭은 고정, 눈금자만 변경. 줌 인/아웃 시 스크롤이 이상함")**: pxPerSec×콘텐츠폭 방식이 근본 문제(줌=물리적 폭 변화→거대 DOM·스크롤 오작동)였음. → **창 `[viewStart, viewStart+viewDur]`** 모델. **레인은 항상 패널 폭 고정**(거대 DOM·가로 스크롤 없음), `pxAt(t)=(t−viewStart)×(폭/viewDur)`. **휠=줌**(viewDur만 변경, 커서 밑 시간 고정, 눈금 단위 자동 적응) · **눈금자 드래그=팬**(viewStart) · **가로 휠=팬**. 보이는 창의 눈금·키만 그림(항상 바운드). 시간 무한(viewStart 자유). clip 전환 시 창 fit.
    - **부수 개선(사용자 요청)**: 시간 표기 **시:분:초**(fmtTick, 782s→13:02) 전역 적용 · **+오브젝트(트랙 추가)를 라벨 상단(눈금자 위치)**으로 · **◆ 선택 후 Del로 삭제**(window capture로 EditorClient 오브젝트삭제보다 먼저, 명시 poseEdit만) + 삭제버튼 제거 · 눈금자 배경 `bg-muted/[0.03]`. tsc 클린. ③**인스펙터 정리** — 타임라인 모드에선 인스펙터의 총길이·이징·반복 숨김(하단 트랜스포트와 중복). tsc 클린.
  - **② per-트랙 독립 타이밍 (2026-07-15, 사용자 브라우저 확인 완료)**: 타임라인의 키 추가/이동/삭제/오토키가 **정렬 포즈(모든 트랙 공유 시간)가 아니라 '그 트랙 하나'에만** 작용 → 트랙마다 키를 서로 다른 시간에 자유 배치(진짜 트랙별 타이밍). **스토어**(`sceneStore`): transient `keySel:{clipId,objectId,idx}`(선택 키) + 액션 `goToKey`(그 트랙만 그 키 값으로 이동+keySel 지정)·`clearKeySel`·`addKeyToTrack`(한 트랙에 현재 트랜스폼 캡처)·`retimeKey`(이웃 클램프, 지연 커밋)·`removeKey`(트랙 최소 1키 유지). **오토키 확장**: `autoKeyPose`가 `keySel` 있으면 **그 트랙의 그 키만** 갱신(없으면 기존 poseEdit 정렬포즈 폴백). goToPose/setPoseEdit는 keySel 해제(모드 배타). **TimelinePanel**: `poseTimes`/`curPoseIdx`(공유 시간축) 로직 제거 → `keySel`/`selTrack`/`selKey`/`totalKeys`/`targetObjId` 기반. 키 하이라이트=keySel 매칭, 우클릭 대상 트랙=행별(`onCtx(e,objectId)`)·눈금자는 `targetObjId`(선택 트랙||root), `beginKey(e,objectId,idx)`·키 클릭=goToKey+selectObject. 간단 모드(poseEdit/goToPose)는 스토어에 그대로 유지(AnimationClipSection 계속 사용). tsc 클린(새 에러 0, 죽은 코드 snap/eqV 정리).
- [ ] P5 다듬기(최단경로·커스텀 피벗·GLB 통일) · 격리 편집모드
