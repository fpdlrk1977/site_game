...
- [x] **P4 타임라인 UI (2026-07-14)**: 인스펙터 Animation에 컴팩트 **타임라인 바** — 키프레임을 시간축에 다이아몬드로. **클릭=그 포즈로 이동**(goToPose·하이라이트), **키 드래그=시간 이동**(setPoseTime 재사용, 이웃 사이 클램프, 마지막 키는 여유+0.5반올림 스케일로 지터↓), **빈 바 드래그=스크럽**(재생헤드를 끌어 중간 동작 미리보기).
  ...
  - **① 이징 곡선 편집(2026-07-15)**: `EasingType`(linear/easeIn/easeOut/easeInOut/backOut/bounceOut) — `animSample.ts`에 곡선 함수맵. **구간(키프레임)별 이징** `AnimKeyframe.easing?`(그 키→다음 키 구간, 미설정 시 클립 기본 폴백). 클립 기본 `AnimClip.easing`도 6종으로 확장(스키마 widen, 하위호환). UI: 타임라인 트랜스포트에 **기본 곡선 + (키 선택 시)구간 곡선** 드롭다운, 인스펙터 이징도 6종. 결정적 테스트 17/17(끝점·키override). tsc 클린. **브라우저 확인 대기.** 남음(선택): ② per-트랙 독립 타이밍(모델 변경, 큼)·격리 편집모드.
